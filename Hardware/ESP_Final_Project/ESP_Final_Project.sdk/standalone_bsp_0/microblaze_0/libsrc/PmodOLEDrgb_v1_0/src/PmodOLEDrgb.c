...
**
** MODIFIED BY ROY KRAVITZ (01-JUL-2016) TO MAP COLORS CORRECTLY TO DISPLAY  
**
*/
uint16_t OLEDrgb_BuildRGB(uint8_t R,uint8_t G,uint8_t B){return ((R>>3)<<6) | ((G>>2)<<0) | ((B>>3)<<11);};
...
**
*/
uint8_t OLEDrgb_ExtractRFromRGB(uint16_t wRGB){return (uint8_t)((wRGB>>6)&0x1F);};
uint8_t OLEDrgb_ExtractGFromRGB(uint16_t wRGB){return (uint8_t)((wRGB>>0)&0x3F);};
uint8_t OLEDrgb_ExtractBFromRGB(uint16_t wRGB){return (uint8_t)((wRGB>>11)&0x1F);};
